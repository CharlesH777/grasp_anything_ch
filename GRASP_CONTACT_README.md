...
本方案面向单机 4 x RTX 3090/4090 服务器。当前目标是在 RealVLG-R1 等数据集上取得可靠的二维接触点和二维碰撞指标，不讨论深度恢复、三维姿态、三维碰撞或真机执行。训练冻结视觉与 LLM 基座，在 LLM attention/MLP 上使用 LoRA，训练视觉到语言的 MLP connector，并仅为两个 grasp 结构 token 增加两个独立的 `2 x hidden_size` 输入/输出 delta；基础 embedding/lm_head 不解冻。
...
- LLM LoRA 的四卡 SDPA + BF16 + ZeRO-2 分阶段启动脚本和 CPU 合成回归测试。
...
- 输入 RGB 图像和一条自然语言指令，输出一个合理的二维两指接触点对。
...
- 不把二维接触线段宣称为物理稳定抓取、米制夹爪开口或 6-DoF 姿态。
...
`<grasp>` 与 `<box>` 都保持六槽 PBD 原子结构，但拥有独立 start/end token，避免把可交换接触点误解释为左上角/右下角。Fast/Hybrid 在块间保持自回归，在一个 grasp block 内通过单次 MTP 前向并行预测四个坐标；结构槽在 contact 模式下受到词表约束。新结构 token 从原 PBD/ref 锚点确定性初始化，checkpoint 保存其独立 token ID。
...
几何过滤和 candidate collision 使用 RealVLG 原始像素浮点坐标。所有候选先量化去重并排除退化项，再按可靠的 outside/collision 标签过滤不安全候选，最后只在安全集合中执行 rectangle IoU medoid 和 FPS 压缩。评测 GT 始终保留原始像素标注，不能用已经量化的训练副本替代。
...
1. 删除重复或近重复候选。
2. 用始终可靠的越界率，以及仅在 `collision_valid=True` 时可靠的碰撞率，删除不安全候选。
3. 按该数据集的评测厚度在原图像素空间为每个安全候选构造 grasp rectangle。
4. 选择与其他安全候选平均 polygon IoU 最高的 IoU-medoid。
5. 对端点做字典序规范化。
...
Phase 0 对每个 GT candidate 用固定 `T=80` 像素计算碰撞率并输出分布。未声明 mask 穷尽时碰撞状态保持 unknown，不按碰撞率删除候选；声明并验证 `collision_valid=True` 后，转换器先删除高碰撞候选，再从剩余安全集合选择 NTP 主标签和 multi-GT 集合，避免基础 token CE 与 pair loss 使用相反标签。
...
```json
{
  ...
  "conversations": [
    {
      "from": "human",
      "value": "Predict one plausible two-finger 2D contact pair for the target described as: the red cup."
    },
    {
      "from": "gpt",
      "value": "<ref>grasp</ref><grasp><136><456><540><880></grasp>"
    }
  ],
...
```json
{
  ...
  "conversations": [
    ...
    {
      "from": "gpt",
      "value": "<ref>grasp</ref><grasp>none</grasp>"
    }
  ],
...
```text
task_type == grasp_contact
target_block[0] == <grasp>
target_block[1:5] 都是坐标 token
target_block[5] == </grasp>
```
...
每个正样本第一版只能包含一个 contact block。增加强断言：
...
`contact_mtp_coord_mask` 标记的是 `labels` 的位置，不是 MTP 输入 mask token 的位置。当前数据管线会在原序列和 `final_mask_targets` 之间插入一个 `bridge_ignore`，新 mask 必须做完全相同的拼接和偏移；偏一位会把监督从 `(x1,y1,x2,y2)` 错移到 `<grasp>` 或 `</grasp>`，这是 64 样本也无法正常过拟合的硬错误。

每个 data-parallel rank 和每个 DataLoader worker 使用永久互斥的数据 ownership，并在自己的 shard 内确定性 shuffle。不能让每个 worker 独立遍历完整数据集，也不能只按会跨 epoch 的全局位置分片；packing 速度不同时，后者仍可能让快 worker 提前重复真实样本。每个参与混合的数据集都必须至少包含 `world_size * workers_per_rank` 条有效样本，避免某些 worker 得到空的 replay/negative shard。dataloader state v6 保存 shard topology、数据/annotation 指纹和影响样本内容的 packing/contact 配置。修复前的 v4/v5 state 只能通过 `MODEL_PATH` 加载权重并启动新 optimizer/数据流，不能作为精确 `RESUME_FROM_CHECKPOINT`。
...
当前前向只对有效监督位置调用真实 hooked `lm_head`，并故意令返回的 `logits=None` 以节省显存。不能改成对全部序列生成 `[L, 152681]` logits。训练 base CE、pair CE 和推理必须复用同一 output-adapter hook，使 `<grasp>`/`</grasp>` 在错误槽位也收到负梯度。
...
image_diag = torch.linalg.vector_norm(image_wh, dim=-1)
whwh = image_wh[:, [0, 1, 0, 1]]
pred_pixels = pred * whwh
target_pixels = target * whwh / 1000.0

pred_center = 0.5 * (pred_pixels[:, :2] + pred_pixels[:, 2:])
gt_center = 0.5 * (target_pixels[:, :2] + target_pixels[:, 2:])
pred_delta = pred_pixels[:, 2:] - pred_pixels[:, :2]
gt_delta = target_pixels[:, 2:] - target_pixels[:, :2]
pred_width = torch.linalg.vector_norm(pred_delta, dim=-1)
gt_width = torch.linalg.vector_norm(gt_delta, dim=-1)

center_per_contact = F.smooth_l1_loss(
    pred_center, gt_center, reduction="none"
).mean(dim=-1) / image_diag
valid_angle = (
    (pred_width.detach() / image_diag > 1e-4)
    & (gt_width / image_diag > 1e-4)
)
...
angle_per_contact = torch.where(
    valid_angle,
    1.0 - torch.where(cosine >= 0.0, cosine, -cosine),
    torch.zeros_like(cosine),
)
width_per_contact = F.smooth_l1_loss(
    pred_width, gt_width, reduction="none"
) / image_diag
```

方向损失天然具有端点交换和 `pi` 周期不变性，不使用 `atan2`。`1-|cos|` 在 90 度处不可微，代码使用显式分支选定一个非零次梯度；这避免旧 `1-cos²` 在最差角度恰好没有梯度。几何量必须先恢复到像素空间，例如 1280x720 图像中的角度不能在正方形 token 网格上直接计算。center/width 必须先做像素空间 SmoothL1，再除图像对角线；若先归一化坐标再进入默认 SmoothL1，普通误差会落入二次区并缩小到约 `1e-6`。角度损失只在预测和 GT 宽度都超过最小门槛后计算。

几何 loss 还需要门控坐标可信度。由完整词表概率计算每个位置的 `P(coord)`，只有四个位置的最小坐标概率超过阈值且坐标条件分布熵不过高时才启用辅助项；门控值使用 `detach()`，防止模型通过降低置信度逃避几何监督。否则即使 Fast 输出仍是文字 token，坐标子词表内部归一化的 soft-argmax 也会产生一个看似有效的伪坐标。最小预测/GT 宽度只门控 angle；置信度足够时 center 和 width 始终保留，否则小开口预测会把负责恢复开口的 width loss 一并关掉。
...
`global_ce_tokens_in_window` 包含被 pair loss 替代的四个 MTP 坐标槽；base CE numerator 排除它们，而 `4 * pair_loss` 把逐 block 均值恢复成 token numerator。若分母也排除这四槽再直接相加两个均值，会把坐标梯度从约 `4/29` 放大到与整段文本等权，阶段切换时产生明显 loss 尺度跳变。
...
若可靠 GT 候选中已有多个无碰撞候选，优先在 multi-GT hard-min 前同时通过 `candidate_collision_2d` 和始终可确定计算的 `candidate_outside_2d` 屏蔽碰撞/越界候选。obstacle 项只在 `collision_valid=True` 时生效，outside 项不依赖 mask；全部候选被屏蔽时不得回退原集合：转换阶段对非穷尽候选跳过样本，仅在 mask 与抓取候选都经审计为穷尽时生成 `ungraspable`，训练阶段若仍收到空集合则直接报数据错误。
...
```text
Predict one plausible two-finger 2D contact pair for the target described as: {query}.
```
...
- 接受 `<grasp><x1><y1><x2><y2></grasp>`。
- 接受 `<grasp>none</grasp>` 并返回空列表；旧 `<box>` 在 grasp 模式下一律判为 strict invalid，checkpoint 迁移必须先升级专用 task token。
- 整个响应只能出现一个 contact block；多个 grasp、`none` 与坐标 grasp 并存或 block 后继续生成几何内容均判为 strict invalid，不能取第一个或挑最优。
- 不排序端点。
...
启动脚本默认要求至少 1000 个 contact 样本、meta 中至少有一个 `grounding` 数据集、全部数据集显式设置 `sampling_weight`，且归一化 replay 权重不低于 15%。目标 20% 仍由 meta 决定；启动日志的归一化权重和 `replay_samples_in_window` 必须同时验证。进入本阶段前，Phase 1 checkpoint 还必须含真实 LoRA/task-adapter 权重和 `phase_acceptance.json`，其中 Fast 格式率至少 99%、四坐标 top-1 至少 95%。小规模 smoke 只能显式覆盖 `CONTACT_MIN_FULL_SAMPLES`，不能仅通过重命名 overfit meta 绕过。
...
先用 `CONTACT_PHASE=negative` 在单候选 geometry 上加入负样本；验收通过后再用 `CONTACT_PHASE=multigt` 保持同一混合比例并只新增 K=8 hard-min。两个阶段都默认要求可信负样本归一化权重至少 1%，只含正例或仅放入一条但几乎采不到的 meta 都会在启动前失败。5%/10% 的正式比例仍按下列顺序逐步调整。
...
coordinate_ce / coordinate_top1_accuracy
...
- 官方转换把全部通过基础数值/边界/非退化校验的原始候选保存在 `evaluation_contact_candidates_pixels`；`--max-candidates` 只限制会话中的代表候选，不得截断 evaluator 的 GT 集合，也不得因二维矩形越界而删除官方正样本。
...
### 14.4 `<grasp>` 与原 PBD 结构先验

抓取任务使用独立 `<grasp>` start/end token，避免与 bbox 左上/右下角共享结构语义；四个坐标槽仍复用 PBD 的并行预测机制和坐标词表。新 token 从已有 PBD/ref 锚点初始化，但拥有独立 ID。
...
- 使用固定 `<ref>grasp</ref>`、独立 `<grasp>` block 和明确的 `grasp_contact` prompt。
- 保留至少 20% grounding/detection replay。
- 分别统计四种坐标象限关系。
- 对 `<box>` 与 `<grasp>` 分别统计格式率，禁止推理 decoder 在 contact 模式回退为 bbox token。
